...
export const useValidation = <S>(validationSchema: ValidationSchema<S>) => {
  ...
  const createValidationsState = (schema: ValidationSchema<S>) => {
    return reduce((prev: any, key: keyof S) => ({
      ...prev,
      [key]: {
        isValid: true,
        error: '',
